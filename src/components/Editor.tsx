...
export default function Editor({ title, initialContent, initialPlaybackLog, onBack, onSave }: EditorProps) {
  ...
  const renderFrame = useCallback((ctx: CanvasRenderingContext2D, entry: PlaybackEntry, progress: number, isExport: boolean = false) => {
    ...
    const fitWidthScale = (w * 0.9) / blockWidth; 
    let scale = 1.0; 
    let tx = 0, ty = 0;

    const focusScale = isExport ? 1.0 : 0.42;

    if (progress > 0.96) {
       // --- FINISH EFFECT: ZOOM OUT (PULL BACK) ---
       ...
       // Interpolate positions from focus point to center
       const cursorY = cursorVisualLine * lineHeight + (lineHeight / 2);
       const startTy = (h / 2.5) - (cursorY * startScale); 
       const startTx = (w / 2) - (blockWidth * startScale / 2);
       
       const endTy = (h / 2) - (totalTextHeight * targetScale / 2);
       const endTx = (w / 2) - (blockWidth * targetScale / 2);
       
       tx = startTx + (endTx - startTx) * t;
       ty = startTy + (endTy - startTy) * t;
    } else {
  ...
  const runExport = async (mode: 'download' | 'share' = 'download', range: 'all' | 'recent100' | 'today' = 'all') => {
    ...
    try {
        ...
        const ZOOM_OUT_DUR = 1500; 
        const STILL_DUR = 5000; 
        const OUTRO_DUR = 3000; 
        const totalVideoDur = INTRO_DUR + totalDuration + ZOOM_OUT_DUR + STILL_DUR + OUTRO_DUR;
        ...
        while (exportVT <= totalVideoDur) {
            ...
            if (exportVT < INTRO_DUR) {
                ...
                const isVisualChange = entryIndex !== lastEntryIndex;

                if (isVisualChange) {
                    // focusScale is 1.0 for export which fits 24px text perfectly inside 720px width
                    renderFrame(ctx as unknown as CanvasRenderingContext2D, entry, 0, true);
            ...
            } else if (exportVT < INTRO_DUR + totalDuration + ZOOM_OUT_DUR) {
                // --- RENDER ZOOM OUT ---
                const zoomVT = exportVT - (INTRO_DUR + totalDuration);
                const zoomProgress = zoomVT / ZOOM_OUT_DUR; 
                
                const entry = processedLog[processedLog.length - 1];
                const mockProgress = 0.96 + (zoomProgress * 0.04);
                
                renderFrame(ctx as unknown as CanvasRenderingContext2D, entry, mockProgress, true);
            ...
            } else if (exportVT < INTRO_DUR + totalDuration + ZOOM_OUT_DUR + STILL_DUR) {
                // --- RENDER STILL PERIOD (5 SECONDS) ---
                const entry = processedLog[processedLog.length - 1];
                ...
                // --- RENDER OUTRO (LOGO) ---
                const outroVT = exportVT - (INTRO_DUR + totalDuration + ZOOM_OUT_DUR + STILL_DUR);
                const outroProgress = outroVT / OUTRO_DUR;
                ...
                // Atmosphere
                ctx.save();
                const gOut = ctx.createRadialGradient(w/2, h/2, 0, w/2, h/2, w*0.8);
                gOut.addColorStop(0, 'rgba(99, 102, 241, 0.1)');
                ...
                // Smooth fade in for outro
                let alpha = 1;
                if (outroProgress < 0.2) alpha = outroProgress / 0.2;
                if (outroProgress > 0.8) alpha = (1 - outroProgress) / 0.2;
                ctx.globalAlpha = Math.max(0, alpha);
                
                // Subtext: Done with
                ctx.fillStyle = 'rgba(255, 255, 255, 0.4)';
                ctx.font = `bold ${Math.floor(w * 0.02)}px "BIZ UDMincho", serif`;
                ctx.fillText('Done with', w / 2, h / 2 - 80);
                
                // Logo: Time×Writer
                ctx.fillStyle = '#FFFFFF';
                // Larger font for branding
                const brandingSize = Math.floor(w * 0.08);
                ctx.font = `bold ${brandingSize}px "BIZ UDMincho", serif`;
                ctx.letterSpacing = "4px";
                ctx.fillText('Time×Writer', w / 2, h / 2);
                
                // Version or accent line
                ctx.fillStyle = '#6366f1';
                ctx.fillRect(w / 2 - 60, h / 2 + 70, 120, 3);
            ...
            try {
                ...
                if (navigator.canShare && navigator.canShare({ files: [file] })) {
                    await navigator.share({
                        title: title || 'Time×Writer Video',
                        text: 'Created with Time×Writer',
                        files: [file]
                    });
                } else {
                    doDownload();
                    showToast('Sharing not supported for this file type.');
                }
            } catch (err) {
        ...
        <div className="h-16 px-4 flex items-center justify-between border-b border-white/5">
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2 relative">
              ...
              <button 
                onClick={() => { onSave(content, playbackLogRef.current); setIsDirty(false); setShowSavedIndicator(true); setTimeout(() => setShowSavedIndicator(false), 2000); }} 
                className="px-5 py-2.5 rounded-full text-[10px] font-bold tracking-[0.2em] flex items-center gap-2 active:scale-95 transition-all text-white shadow-lg"
                style={{ 
                  backgroundColor: isDirty ? '#4A90E2' : '#555555',
                  boxShadow: isDirty ? '0 4px 12px rgba(74, 144, 226, 0.3)' : 'none'
                }}
              >
